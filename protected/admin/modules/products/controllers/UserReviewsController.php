...
class UserReviewsController extends Controller {
        ...
        public function init() {
                if (!isset(Yii::app()->session['post']['static_pages']) || Yii::app()->session['post']['static_pages'] != 1) {
        ...
        public function actionCreate() {
                ...
                if (isset($_POST['UserReviews'])) {
                        ...
                        $image1 = CUploadedFile::getInstance($model, 'user_image');
                        $model->user_image = $image1->extensionName;
                        if ($model->validate()) {
                                if ($model->save()) {
                                        if ($image1 != '') {
                                                $image1->saveAs(Yii::app()->basePath . '/../uploads/review-image/' . $model->id . '.' . $image1->extensionName);
                                        }
                                        else {
        ...
        public function actionDelete($id) {
                $model = $this->loadModel($id);
                if ($model->user_image != "") {
                        unlink(Yii::app()->basePath . "/../uploads/review-image/" . $model->id . "." . $model->user_image);
                        $model->delete();
                }
                else {
                        $model->delete();
