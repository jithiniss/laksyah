...
//$subtotoal = $subtotoal + $total;
        ...
        public function actionBuynow() {
                ...
                $product_option_id = OptionDetails::model()->findByAttributes(['master_option_id' => $master_option_id, 'size_id' => $option_size, 'color_id' => $option_color]);
                ...
                if (!empty($cart)) {
                        ...
                        $model->product_id = $id;
                        $model->quantity = $qty;
                        $model->options = $product_option_id->id;
...
//cart empty //
        ...
        public function giftCardCheck($gift_card, $cart) {
                $amount = $this->total($cart);
                if ($amount > $gift_card->amount || $amount == $gift_card->amount) {
                        $gift_card->amount = 0;
                        $gift_card->save();
                        return true;
                } else {
                        $gift_card->amount = $gift_card->amount - $amount;
                        $gift_card->save();
                        Yii::app()->user->setFlash('success', "Your card balance is $gift_card->amount");
                        return true;
                }
        }
